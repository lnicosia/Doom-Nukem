...
/*   Updated: 2019/09/10 18:28:43 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_env *env)
{
	...
	skybox.clipped_alpha = (x - v1.clipped_x) / v1.clipped_xrange;
	skybox.z = 1.0 / ((1.0 - skybox.alpha) / v1.vz
			+ skybox.alpha / v2.vz);
	skybox.clipped_z = 1.0 / ((1.0 - skybox.clipped_alpha)
			/ v1.clipped_vz + skybox.clipped_alpha / v2.clipped_vz);
	...
	skybox.max_ceiling = skybox.clipped_alpha * v1.ceiling_range + v1.c;
	skybox.current_ceiling = ft_clamp(skybox.max_ceiling,
			env->ymin[x], env->ymax[x]);
	skybox.max_floor = skybox.clipped_alpha * v1.floor_range + v1.f;
