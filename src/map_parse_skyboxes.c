...
int     check_skyboxes_number_validity(t_map_parser *parser, t_env *env)
{
	...
	line++;
	if (*line && *line != 'K')
		return (ft_perror("Expected letter: 'K'\n"));
	line++;
	if (*line && *line != 'Y')
		return (ft_perror("Expected letter: 'Y'\n"));
	line++;
	if (*line && *line != ' ')
		return (ft_perror("Expected a space\n"));
	line++;
	if (valid_int(line, parser))
		return (ft_perror("Invalid int for skybox images number\n"));
...
int		map_parse_skyboxes(t_env *env, t_map_parser *parser)
{
	...
	while (i < env->resource.nb_skyboxes)
	{
		if (parse_bmp_file(env, parser))
			return (ft_perror("Error while parsing skyboxes\n"));
		i++;
	}
	return (0);
}