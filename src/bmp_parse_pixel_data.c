...
/*   Updated: 2020/01/09 12:04:57 by lnicosia         ###   ########.fr       */
/*																			*/
/* ************************************************************************** */
...
int index, t_env *env)
{
	...
	if ((ret = read(fd, str, size)) > 0)
	{
		...
		while (byte + 4 < ret)
		{
			env->ui_textures[index].str[x + y * parser->w] = get_pixel(byte,
			str, parser); 
			byte += parser->bpp / 8.0;
			x++;
			if (x >= parser->w)
...
int index, t_env *env)
{
	...
	if ((ret = read(fd, str, size)) > 0)
	{
		...
		while (byte + 4 < ret)
		{
			env->mini_enemies_textures[index].str[x + y * parser->w] = get_pixel(byte,
			str, parser); 
			byte += parser->bpp / 8.0;
			x++;
			if (x >= parser->w)
...
int index, t_env *env)
{
	...
	if ((ret = read(fd, str, size)) > 0)
	{
		...
		while (byte + 4 < ret)
		{
			env->mini_objects_textures[index].str[x + y * parser->w] = get_pixel(byte,
			str, parser); 
			byte += parser->bpp / 8.0;
			x++;
			if (x >= parser->w)
...
int index, t_env *env)
{
	...
	if ((ret = read(fd, str, size)) > 0)
	{
		...
		while (byte + 4 < ret)
		{
			env->mini_skyboxes[index].str[x + y * parser->w] = get_pixel(byte,
			str, parser); 
			byte += parser->bpp / 8.0;
			x++;
			if (x >= parser->w)
