...
/*   Updated: 2019/11/15 18:37:34 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_env *env)
{
	...
	zbuffer = env->zbuffer;
	map_lvl = env->textures[sector.ceiling_texture].nb_maps - 1;
	i = vline.start;
	while (i <= vline.end)
	{
		...
		if (!env->options.test)
		{
			...
			/ pow(2, env->textures[sector.ceiling_texture].nb_maps - 1 -map_lvl)
