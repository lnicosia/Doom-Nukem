...
/*   Updated: 2020/05/19 12:24:48 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	read_data(t_map_parser *parser, int ret)
{
	int	size;
	...
	if ((ret = read(parser->fd, parser->tmp, size + 1)) < size + 1)
		return (custom_error("The file is smaller than the given size file\n"));
	ft_strdel(&(parser->tmp));
	return (0);
}
...
int	skip_file(t_map_parser *parser)
{
	...
	if (read_data(parser, ret))
		return (custom_error("Could not skip the file\n"));
	return (0);
}
