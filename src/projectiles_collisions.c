...
double radius)
{
	...
	while (i < env->nb_objects)
	{
		if (env->objects[i].health > 0
			...
			+ radius && env->objects[i].exists
			&& env->objects[i].solid && pos.z <= env->objects[i].height
			+ env->objects[i].pos.z && pos.z >= env->objects[i].pos.z)
		{
			distance = distance_two_points_2d(env->objects[i].pos.x,
			env->objects[i].pos.y, pos.x, pos.y);
		...
		if (hitbox_collision(new_v2(pos.x, pos.y), new_v2(dest.x, dest.y),
			...
			radius + env->objects[i].size_2d) && env->objects[i].exists
			&& env->objects[i].solid && pos.z <= env->objects[i].height
			+ env->objects[i].pos.z && pos.z >= env->objects[i].pos.z)
		{
			distance = distance_two_points_2d(env->objects[i].pos.x,
