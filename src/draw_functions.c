...
/*   Updated: 2019/05/23 11:43:11 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_vline_floor(t_vline vline, t_render render, t_env *env)
{
	...
	while (i <= vline.end)
	{
		...
		{
			yalpha = (i - render.max_floor) / (double)(env->h - render.max_floor);
...
void	draw_vline(t_vline vline, t_render render, t_env *env)
{
	...
	x = (render.alpha * (env->sdl.image->w * render.wall_size / render.vz2)) / ((1 - render.alpha) * (1 / render.vz1) + render.alpha * (1 / render.vz2));
