...
/*   Updated: 2019/05/10 18:31:59 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (env.running)
	{
		...
		if (env.options.test)
		{
			...
			if (env.options.render_type)
				print_text(new_v2(env.h - 50, 0), new_printable_text("Rendering from camera", "fonts/amazdoom/AmazDooMLeft.ttf", 0xFFFFFFFF, 30), &env);
			else
				print_text(new_v2(env.h -50, 0), new_printable_text("Rendering from player", "fonts/amazdoom/AmazDooMLeft.ttf", 0xFFFFFFFF, 30), &env);
		}
		update_screen(&env);
		// BMP parser
