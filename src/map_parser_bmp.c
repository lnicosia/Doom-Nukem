...
int		check_file_validity(t_map_parser *parser)
{
	...
	ft_strdel(&(parser->tmp));
	if (size < 54)
		return (ft_perror("Invalid size for bmp file, size is too small\n"));
...
int		parse_bmp_file(t_env *env, t_map_parser *parser)
{
	if (parse_file_name(parser))
		return (ft_perror("Error while parsing the file name\n"));
	...
	{
		if (skip_file(parser))
			return (ft_perror("Error while skipping the resource\n"));
	}
	return (0);
}
