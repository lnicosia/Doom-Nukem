...
void	draw_current_weapon(int i, int hud_start, int weapon_size, t_env *env)
{
	int		height;
	int		rectangle_size;
	void	(*func)(t_sprite, t_point, t_point, t_env *);
	...
	if (env->weapons[i].possessed)
		...
		func = apply_sprite_grey;
	func(env->weapons[i].sprite, new_point(
	env->h - env->sprite_textures[ARMOR_LIFE_HUD].surface->h / 2 - height / 2,
	hud_start + (weapon_size + weapon_size / (NB_WEAPONS + 1)) * i
	+ weapon_size / (NB_WEAPONS + 1)),
