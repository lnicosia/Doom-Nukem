...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw_ceiling_bullet_holes_color.c                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
...
void	draw_current_ceiling_bullet_hole_color(t_drawer *drawer, t_env *env)
{
...
void	draw_ceiling_bullet_hole_color(t_bullet_hole *curr, t_drawer *drawer,
t_env *env)
{
	Uint32		*sprite_pixels;
	...
	* curr->scale.y + drawer->sprite->start[0].y;
	if (drawer->sprite_x >= drawer->sprite->start[0].x
		...
		* (int)drawer->sprite_y] != 0xFFC10099)
		draw_current_ceiling_bullet_hole_color(drawer, env);
}


void	draw_ceiling_bullet_holes_color(t_sector *sector, t_drawer *drawer,
t_env *env)
{
	t_bullet_hole	*curr;
	t_list			*bullet_holes;
	...
	while (bullet_holes)
	{
		...
		draw_ceiling_bullet_hole_color(curr, drawer, env);
