...
int		print_results(t_env *env)
{
	...
	if (print_text(new_point(200, 320), new_printable_text("Score :",
	...
	if (env->player.killed > 1)
		...
		env->player.killed);
	if (print_text(new_point(280, 220), new_printable_text(env->snprintf,
	env->sdl.fonts.lato30, 0xFFFFFFFF, 30), env))
		return (-1);
	ft_snprintf(env->snprintf, SNPRINTF_SIZE, "Accuracy: %.f%%",
	env->player.accuracy);
	if (print_text(new_point(360, 220), new_printable_text(env->snprintf,
	env->sdl.fonts.lato30, 0xFFFFFFFF, 30), env))
		return (-1);
	return (0);
}
