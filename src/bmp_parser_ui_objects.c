...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/29 12:04:17 by sipatry           #+#    #+#             */
/*   Updated: 2020/01/29 14:29:54 by sipatry          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_mini_objects_textures(int fd, int index, t_env *env)
{
	t_bmp_parser	parser;
...
//	check_bmp_parsing(parser);
	ft_printf("{red}");
	if (!(env->mini_objects_textures[index].surface = SDL_CreateRGBSurfaceWithFormat(
					0,
					parser.w,
					parser.h,
					parser.bpp,
	...
	env->mini_objects_textures[index].str = env->mini_objects_textures[index].surface->pixels;
	env->mini_objects_textures[index].scale = 1;
	env->mini_objects_textures[index].xpadding = 0;
	env->mini_objects_textures[index].ypadding = 0;
	...
	//check_bmp_parsing(parser);
	if (parse_pixel_data_ui_mini_objects(fd, &parser, index, env))
...
int			parse_bmp_mini_objects_textures(char *file, int index, t_env *env)
{
	...
	if (parse_mini_objects_textures(fd, index, env))
