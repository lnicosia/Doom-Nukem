...
/*   Updated: 2019/09/27 17:51:56 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_render render, t_env *env)
{
	...
	while (x <= render.xend)
	{
		...
		env->wall_texel[x].x = env->alpha[x] * render.texture_scale.x
			* env->z[x] + sector.align[render.i].x;
		if (!(env->wall_texel[x].x != env->wall_texel[x].x))
		{
			while (env->wall_texel[x].x >= texture_w)
				env->wall_texel[x].x -= texture_w;
			while (env->wall_texel[x].x < 0)
				env->wall_texel[x].x += texture_w;
		}
