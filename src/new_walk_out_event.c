...
/*   Updated: 2020/01/15 17:50:14 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_events_parser *eparser)
{
	...
	if (sector_parser(env, parser, line, eparser))
		return (-1);
	if (!(env->sectors[eparser->trigger_sector].walk_out_event = (t_event*)ft_realloc(
		env->sectors[eparser->trigger_sector].walk_out_event,
		sizeof(t_event) * env->sectors[eparser->trigger_sector].nb_walk_out_events,
		sizeof(t_event) * (env->sectors[eparser->trigger_sector].nb_walk_out_events
		+ 1))))
