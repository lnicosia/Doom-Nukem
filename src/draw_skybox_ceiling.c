...
t_skybox_data wall_data, t_env *env)
{
	...
	if (wall_data.mode == CEILING)
		...
		ceiling_texture) - 1].textures[1];
	else if (wall_data.mode == FLOOR)
		texture = &env->skyboxes[abs(env->sectors[render->sector->num].
		floor_texture) - 1].textures[1];
	else
		texture = &env->skyboxes[abs(env->sectors[render->sector->num].
		textures[wall_data.i]) - 1].textures[1];
	drawer->texture_w = texture->surface->w;
	drawer->texture_h = texture->surface->h;
	drawer->texture_pixels = texture->str;
}

void	get_skybox_ceiling_z(t_drawer *drawer, t_render *render,
t_skybox_data wall_data)
{
	drawer->alpha =
	render->ceiling_start / (double)(drawer->i - render->ceiling_horizon);
	if (wall_data.mode == CEILING)
	{
		drawer->salpha =
...
void	click_on_skybox_ceiling(t_render *render, t_skybox_data wall_data,
t_env *env)
{
	if (env->playing)
	  	reset_hover(env);
...
void	put_skybox_ceiling_pixel(t_drawer *drawer, t_render *render,
t_skybox_data wall_data, t_env *env)
{
...
t_render *render, t_env *env)
{
	...
	while (drawer.i <= drawer.vline.end)
	{
		...
		if ((env->editor.tab && drawer.vline.x == env->sdl.mx
			...
			&& drawer.i == env->h_h))
		{
			click_on_skybox_ceiling(render, wall_data, env);
		}
		put_skybox_ceiling_pixel(&drawer, render, wall_data, env);
		drawer.i++;
	}
}
