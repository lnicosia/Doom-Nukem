...
/*   Updated: 2020/02/18 18:30:03 by sipatry          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_wall_sprite_arrays(t_env *env)
{
	...
	if (env->selected_wall_sprite_wall != -1)
	{
		...
		wall = &sector->wall_sprites[env->selected_wall_sprite_sprite];
		index = env->selected_wall_sprite_wall;
	}
	if (env->editor.selected_wall != -1)
	{
		sector = &env->sectors[env->editor.selected_sector];
		wall = &env->sectors[env->editor.selected_sector].wall_sprites[env->editor.selected_wall];
	...
	* (wall->nb_sprites + 1))))
		return (-1);
	wall->nb_sprites++;
	if (set_camera_sprites_array(&env->player.camera, env->editor.selected_sector, index, env))
		return (-1);
	return (0);
}