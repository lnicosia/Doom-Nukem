...
/*   Updated: 2020/04/29 17:22:55 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_current_ceiling_sprite_color(int j, t_render *render,
...
void	draw_ceiling_sprite_color(int j, t_render *render,
t_drawer *drawer, t_env *env)
{
	Uint32		*sprite_pixels;
	...
	* drawer->sector->ceiling_sprites_scale[j].y + drawer->sprite->start[0].y;
	if (drawer->sprite_x >= drawer->sprite->start[0].x
		...
		* (int)drawer->sprite_y] != 0xFFC10099)
		draw_current_ceiling_sprite_color(j, render, drawer, env);
}

void	draw_ceiling_sprites_color(t_sector *sector, t_render *render,
t_env *env)
{
	int			j;
	t_drawer	drawer;
