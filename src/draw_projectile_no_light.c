...
/*   Updated: 2020/01/16 18:00:54 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		*projectile_loop_no_light(void *param)
{
	t_env				*env;
	...
	while (++x <= xend)
	{
		...
		if (sprite->reversed[prender->index])
			xalpha = 1.0 - xalpha;
		textx = (1.0 - xalpha) * sprite->start[prender->index].x + xalpha
		* sprite->end[prender->index].x;
		y = prender->ystart;
		while (y < yend)
		{
			yalpha = (y - prender->y1) / prender->yrange;
			texty = (1.0 - yalpha) * sprite->start[prender->index].y + yalpha
			* sprite->end[prender->index].y;
			if ((projectile->rotated_pos.z < zbuffer[x + y * env->w]
						&& texture_pixels[textx + texty * texture.surface->w]
						!= 0xFFC10099))
			{
				pixels[x + y * env->w] = texture_pixels[textx + texty
				* texture.surface->w];
...
static int	threaded_projectile_loop_no_light(t_projectile *projectile,
t_render_projectile *prender, t_env *env)
{
	...
	while (i < THREADS)
	{
		...
		/ (double)THREADS * (i + 1);
		if (pthread_create(&threads[i], NULL, projectile_loop_no_light,
		  	&pt[i]))
			return (-1);
		i++;
	}
