...
static void		threaded_projectile_loop_no_light(t_projectile projectile, t_render_projectile prender, t_env *env)
{
	...
	while (i < THREADS)
	{
		...
		pthread_create(&threads[i], NULL, projectile_loop_no_light, &pt[i]);
		i++;
	}
	while (i-- > 0)
...
void		draw_projectile_no_light(t_camera camera, t_projectile *projectile,
t_env *env)
{
	t_render_projectile	prender;
	t_sprite			sprite;
	...
	threaded_projectile_loop_no_light(*projectile, prender, env);
}
