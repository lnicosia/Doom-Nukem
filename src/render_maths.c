...
/*   Updated: 2019/05/22 11:29:16 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	project_floor_and_ceiling(t_render *render, t_env *env, t_sector sector, int i)
{
	...
	render->floor1 = env->h / 2 +
		(int)((sector.clipped_floors1[i] - env->player.pos.z + render->clipped_vz1 * env->player.angle_z)
				* (vfov / -render->clipped_vz1));
	render->floor2 = env->h / 2 +
		(int)((sector.clipped_floors2[i + 1] - env->player.pos.z + render->clipped_vz2 * env->player.angle_z)
				* (vfov / -render->clipped_vz2));
	render->ceiling1 = env->h / 2 +
		(int)((sector.clipped_ceilings1[i] - env->player.pos.z + render->clipped_vz1 * env->player.angle_z)
				* (vfov / -render->clipped_vz1));
	render->ceiling2 = env->h / 2 +
		(int)((sector.clipped_ceilings2[i + 1] - env->player.pos.z + render->clipped_vz2 * env->player.angle_z)
				* (vfov / -render->clipped_vz2));
	render->x1 = env->w / 2 + (int)(render->clipped_vx1 * (hfov / -render->clipped_vz1));
	render->x2 = env->w / 2 + (int)(render->clipped_vx2 * (hfov / -render->clipped_vz2));
}
...
void	project_neighbor_floor_and_ceiling(t_render *render, t_env *env, t_sector neighbor)
{
	...
	render->neighbor_floor1 = env->h / 2 +
		(int)((neighbor.clipped_floors1[render->nv1] - env->player.pos.z + render->clipped_vz1 * env->player.angle_z)
				* (vfov / -render->clipped_vz1));
	render->neighbor_floor2 = env->h / 2 +
		(int)((neighbor.clipped_floors2[render->nv2] - env->player.pos.z + render->clipped_vz2 * env->player.angle_z)
				* (vfov / -render->clipped_vz2));
	render->neighbor_ceiling1 = env->h / 2 +
		(int)((neighbor.clipped_ceilings1[render->nv1] - env->player.pos.z + render->clipped_vz1 * env->player.angle_z)
				* (vfov / -render->clipped_vz1));
	render->neighbor_ceiling2 = env->h / 2 +
		(int)((neighbor.clipped_ceilings2[render->nv2] - env->player.pos.z + render->clipped_vz2 * env->player.angle_z)
				* (vfov / -render->clipped_vz2));
}
