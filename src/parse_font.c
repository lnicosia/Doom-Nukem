...
int		check_font_validity(t_map_parser *parser)
{
	...
	if (valid_int(parser->line, parser))
		return (ft_perror("Invalid size for bmp file\n"));
	size = ft_atoi(parser->line);
	if (create_font_file(parser, size))
		return (ft_perror("Error while creating font from map resources\n"));
	return (0);
}
...
int		parse_font_file(t_env *env, t_map_parser *parser)
{
	if (parse_font_name(parser))
		return (ft_perror("Error while parsing font name\n"));
	if (check_existing_fonts(env, parser->resource_name))
	{
		...
		if (check_font_validity(parser))
			return (ft_perror("Error while checking and creating file\n"));
	}
	else
	{
		if (skip_file(parser))
			return (ft_perror("Error while skipping the font file\n"));
	}
	return (0);
}
