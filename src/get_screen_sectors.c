...
/*   Updated: 2019/09/18 11:16:35 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*get_screen_sectors_loop(void *param)
{
	...
	while (x < end)
	{
		tmp = (x / (double)(env->w - 1)) * env->player.camera.range + env->player.camera.near_left;
		curr.y = -env->player.camera.near_z;
		curr.x = tmp * -env->player.camera.angle_sin - curr.y * env->player.camera.angle_cos + env->player.pos.x;
		curr.y = tmp * env->player.camera.angle_cos - curr.y * env->player.camera.angle_sin + env->player.pos.y;
		curr.z = env->player.head_z;
