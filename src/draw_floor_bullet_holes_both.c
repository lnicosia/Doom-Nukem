...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw_floor_bullet_holes_both.c                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
...
void	draw_current_floor_bullet_hole_both(t_drawer *drawer, t_env *env)
{
...
void	draw_floor_bullet_hole_both(t_bullet_hole *curr, t_drawer *drawer,
t_env *env)
{
	Uint32		*sprite_pixels;
	...
	* curr->scale.y + drawer->sprite->start[0].y;
	if (drawer->sprite_x >= drawer->sprite->start[0].x
		...
		* (int)drawer->sprite_y] != 0xFFC10099)
		draw_current_floor_bullet_hole_both(drawer, env);
}


void	draw_floor_bullet_holes_both(t_sector *sector, t_drawer *drawer,
t_env *env)
{
	t_bullet_hole	*curr;
	t_list			*bullet_holes;
	...
	while (bullet_holes)
	{
		...
		draw_floor_bullet_hole_both(curr, drawer, env);
