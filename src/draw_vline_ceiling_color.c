...
/*   Updated: 2020/03/05 18:44:40 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_vline_ceiling_color(t_sector sector, t_vline vline, t_render render,
		t_env *env)
{
	int		i;
	Uint32	*pixels;
	...
	while (i <= vline.end)
	{
		...
		{
			pixels[coord] = apply_light_color(texture_pixels[(int)text_x + render.texture_w * (int)text_y], sector.light_color, sector.intensity);
