...
int	check_wall_textures5(t_env *env, int fd)
{
...
int	check_wall_textures4(t_env *env, int fd)
{
	...
	if (!env->init.textures[12] && close(fd))
		return (custom_error("Could not close the file in check textures\n"));
	return (check_wall_textures5(env, fd));
}

int	check_wall_textures3(t_env *env, int fd)
{
	...
	if (!env->init.textures[9] && close(fd))
		return (custom_error("Could not close the file in check textures\n"));
	return (check_wall_textures4(env, fd));
}

int	check_wall_textures2(t_env *env, int fd)
{
	...
	if (!env->init.textures[6] && close(fd))
		return (custom_error("Could not close the file in check textures\n"));
	return (check_wall_textures3(env, fd));
}
...
int	check_wall_textures(t_env *env)
{
	...
	if (!env->init.textures[2] && close(fd))
		return (custom_error("Could not close the file in check textures\n"));
	return (check_wall_textures2(env, fd));
}
