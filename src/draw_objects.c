...
static void		*object_loop(void *param)
{
	...
	while (++y < yend)
	{
		...
		texty = (1.0 - yalpha) * sprite->start[orender->index].y + yalpha
		* sprite->end[orender->index].y;
		...
		while (x < xend)
		{
			...
			if (sprite->reversed[orender->index])
				xalpha = 1.0 - xalpha;
			textx = (1.0 - xalpha) * sprite->start[orender->index].x + xalpha
			* sprite->end[orender->index].x;
				...
				0xFFC10099))
			{
				...
				if (!env->editor.select && env->selected_object == object->num)
					pixels[x + y * env->w] = blend_alpha(pixels[x + y * env->w],
					0x1abc9c, 128);
...
static int	threaded_object_loop(t_object *object, t_render_object *orender,
t_env *env)
{
