...
/*   Updated: 2019/08/19 15:06:34 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		init_objects(t_env *env, t_map_parser *parser)
{
	...
	while ((parser->ret = get_next_line(parser->fd, &tmp)))
	{
		...
		{
			if (*line != 'O')
				return (invalid_char("objects number", "'O'", *line, parser));
			line++;
			if (!*line)
				return (missing_data("objects number", parser));
			if (*line != ' ')
				return (invalid_char("objects number", "space of a digit",
							*line, parser));
			...
			if (*line && *line == ' ')
				return (extra_data("objects number declaration", parser));
			if (*line)
				return (invalid_char("objects number", "space of a digit",
							*line, parser));
			if (env->nb_objects < 0)
			...
			if (env->nb_objects
					&& !(env->objects = (t_object*)malloc(sizeof(t_object)
							* (env->nb_objects))))
				return (custom_error("Could not malloc objects!"));
			ft_strdel(&tmp);
			return (0);
		}
		else if (line[0] != '#')
