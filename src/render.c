...
/*   Updated: 2019/11/15 10:07:26 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_env *env)
{
	...
	{
		camera->v[sector->num][i].texture_scale.x = (1 / sector->scale[i].x) * (sector->wall_width[i] / 10.0) / camera->v[sector->num][i + 1].vz;
		j = 0;
	...
	{
		camera->v[sector->num][i].texture_scale.x = (1 / sector->scale[i].x) * (sector->wall_width[i] / 10.0)
