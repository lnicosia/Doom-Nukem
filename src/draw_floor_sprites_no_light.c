...
/*   Updated: 2020/04/29 17:30:30 by lnicosia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_current_floor_sprite_no_light(int j, t_render *render,
...
void	draw_floor_sprite_no_light(int j, t_render *render, t_drawer *drawer,
t_env *env)
{
	Uint32		*sprite_pixels;
	...
	* drawer->sector->floor_sprites_scale[j].y
	...
	if (drawer->sprite_x >= drawer->sprite->start[0].x
		...
		* (int)drawer->sprite_y] != 0xFFC10099)
		draw_current_floor_sprite_no_light(j, render, drawer, env);
}

void	draw_floor_sprites_no_light(t_sector *sector, t_render *render,
t_env *env)
{
	int			j;
	t_drawer	drawer;
