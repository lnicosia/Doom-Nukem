...
static unsigned int bit_read_int(unsigned char *buf, int *offset)
{
	unsigned int val = /* 16 bit value */
		(((unsigned int) buf[*offset]  ) << 8)
	|	 ((unsigned int) buf[*offset+1]);
	*offset += 2;
	return val;
}
	...
	*/
	...
	if(fr->bsbuf[lame_offset] != 0)
	{
		...
		for(i =0; i < 2; ++i)
		{
			...
			unsigned int gain  = bit_read_int(fr->bsbuf, &lame_offset) & 0x1ff; /* 19 in (2 cycles) */
